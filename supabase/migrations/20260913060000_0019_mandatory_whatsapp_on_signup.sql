-- 0019 — El alta con Google exige WhatsApp y consentimiento.
--
-- Objetivo del proyecto: mandar promociones por WhatsApp. Sin numero eso no ocurre, y con el alta
-- pidiendo solo el apodo llevabamos 1 telefono en 176 perfiles. A partir de aqui, crear cuenta exige
-- apodo + telefono + consentimiento explicito, y el telefono queda fijo.
--
-- El candado vive aqui y no en el formulario: quitar un campo del cliente no impide que alguien llame
-- la RPC desde la consola del navegador.

-- ─── 1. Un numero, una cuenta ────────────────────────────────────────────────
-- Sumado a que el telefono es inmutable, cierra la puerta a registrar varias cuentas con distintos
-- Gmail y el mismo telefono para acumular puntos del mismo Live. Parcial porque los 175 cadetes
-- legacy no tienen telefono y varios null no deben chocar entre si.
create unique index if not exists profiles_phone_key
  on public.profiles (phone)
  where phone is not null;

-- ─── 2. complete_signup(): apodo + telefono + consentimiento, o nada ─────────
-- Sustituye a claim_username. Todo en una transaccion: no puede quedar un apodo tomado sin telefono.
...
-- ─── 3. Fuera claim_username ─────────────────────────────────────────────────
-- Es el punto clave de la migracion: si se queda, sigue siendo una puerta abierta para conseguir apodo
-- sin dejar telefono, y todo lo anterior se vuelve decorativo.
drop function if exists public.claim_username(text);

-- ─── 4. update_my_profile(): el telefono se escribe una sola vez ─────────────
-- Identica a la anterior salvo por el candado del telefono. Se acepta cuando el perfil aun no tiene
-- (los 175 legacy pueden agregar el suyo por primera vez) y se rechaza cualquier cambio posterior.
-- whatsapp_opt_in sigue siendo editable a proposito: el aviso de privacidad promete poder revocar.
