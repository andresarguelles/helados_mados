    ...
    <div className="min-h-screen flex flex-col bg-brand-crema">
      ...
      <div className="flex-1 max-w-lg mx-auto px-4 pt-20 pb-8">
        <div className="flex items-center gap-3 mb-6 pt-4">
          <Shield className="w-8 h-8 text-brand-fresa" />
          <h1 className="font-heading text-brand-tinta text-2xl">Términos y Condiciones</h1>
        </div>

        <div className="flex flex-col gap-5">

          <Section icon={<Clock className="w-5 h-5" />} title="Sujeto a disponibilidad">
            Todos los premios físicos (helados, paletas, toppings) están limitados al inventario disponible en la sucursal al momento del canje. Helados Mados no garantiza existencia ilimitada de ningún premio y puede agotar el stock en cualquier momento sin previo aviso.
          </Section>
          ...
          <Section icon={<AlertTriangle className="w-5 h-5" />} title="Reglas de participación">
            <ul className="list-disc list-inside flex flex-col gap-1.5 text-sm">
              ...
              <li>Máximo 3 canjes distintos por dirección IP por cada dinámica activa.</li>
              ...
              <li>Helados Mados puede modificar, suspender o cancelar cualquier dinámica en cualquier momento.</li>
            </ul>
          </Section>
  ...
  return (
    <div className="paper-card rounded-3xl p-5">
      <div className="flex items-center gap-2 mb-3">
        <span className="text-brand-fresa">{icon}</span>
        <h2 className="font-heading text-brand-tinta text-base">{title}</h2>
      </div>
      <div className="font-body text-brand-tinta/70 text-sm leading-relaxed">{children}</div>
    </div>
  )
}
